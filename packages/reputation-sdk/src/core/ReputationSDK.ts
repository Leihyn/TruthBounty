...
export class ReputationSDK {
   ...
   */
  async getTruthScore(walletAddress: string): Promise<TruthScore> {
    ...
    const truthScore = this.scoringEngine.calculateTruthScore(
      walletAddress,
      platformStats
    );
   ...
   */
  async subscribeAll(callback: (event: AdapterEvent) => void | Promise<void>): Promise<void> {
    for (const adapter of this.adapters) {
      try {
   ...
   */
  async getLeaderboard(limit = 100, offset = 0): Promise<TruthScore[]> {
    if (!this.storage) {
