...
/**
 * Kalshi Trade Resolution
 *
 * Resolves pending simulated trades by querying Kalshi API.
...
export async function GET() {
  ...
  try {
    ...
    if (fetchError) {
      ...
      if (fetchError.code === '42P01' || fetchError.message?.includes('does not exist')) {
        return NextResponse.json({
          ...
          resolved: 0,
          pending: 0,
          message: 'Kalshi simulated trades table not configured',
    ...
    return NextResponse.json({
      ...
      message: 'Kalshi resolution ready - betting coming soon',
      duration: Date.now() - startTime,
    });
  } catch (error: any) {
    console.error('Kalshi resolve error:', error);
    return NextResponse.json({
      success: false,
