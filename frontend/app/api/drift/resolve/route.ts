...
/**
 * Drift BET Trade Resolution
 *
 * Resolves pending simulated trades by querying Drift Data API.
...
export async function GET() {
  ...
  try {
    ...
    if (fetchError) {
      ...
      if (fetchError.code === '42P01' || fetchError.message?.includes('does not exist')) {
        return NextResponse.json({
          ...
          resolved: 0,
          pending: 0,
          message: 'Drift simulated trades table not configured',
    ...
    return NextResponse.json({
      ...
      message: 'Drift resolution ready - betting coming soon',
      duration: Date.now() - startTime,
    });
  } catch (error: any) {
